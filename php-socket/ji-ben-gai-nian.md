...
protocol：故名思意 , 就是指定协议 , 当然是设置指定`domain`套接字下的具体协议 . 在PHP中可以使用getprotobynumber\(\)函数获取与协议名称关联的协议编号 , 即获取系统中/etc/protocol中的信息 , 相关的还有一个getprotobyname\(\) , 获取名字 , 如果你有自定义的协议名 , 也可以写在/etc/protocol里 . 
