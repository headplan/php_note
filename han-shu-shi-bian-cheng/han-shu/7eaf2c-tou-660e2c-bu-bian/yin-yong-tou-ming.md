...
Memoization是一种技术 , 您可以缓存给定参数集的函数结果 , 以便您不必在下次调用时再次执行 . 我们将在第8章“性能优势”中详细介绍 . 现在 , 让我说如果你的语言具有引用透明性的表达方式 , 它可以在需要时自动执行记忆 . 

这意味着它可以根据调用频率和各种其他参数来决定是否值得自动 memoizing 一个函数, 而无需任何开发者的干预或暗示 . 
